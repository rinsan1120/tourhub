...
- `umap_backup_map.umap` と `P35-18_Roadside_Station.geojson` は置換更新される原本データであり、通常の改修では直接編集しない。
- 両マスタへの許可された更新は、正規マスタによるファイル全体の置換のみとする。
- 地図データに関する機能・性能要件は、原本を加工せず読み込み処理側で対応する。
